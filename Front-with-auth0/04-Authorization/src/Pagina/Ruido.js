...
class Ruido extends Component {
	...
	deleteRuido() {
	    ...
	    axios.delete(`${API_URL}/sonido/${this.props.id}`, { credentials: true, headers: headers })
		    .then(response => { 
	...
	updateRuido() {
		const { getIdToken } = this.props.auth;
		const ruido = { id: this.props.id, name: this.state.name, code: this.state.code };
	    const headers = { Authorization: `Bearer ${getIdToken()}`};
	    axios.put(`${API_URL}/sonido`, ruido, { credentials: true, headers: headers })
		    .then(response => { 
